...
assert_lowercase <- assert_create(
  func = function(x) {x == tolower(x)}, # A function that returns TRUE/FALSE, depending on whether assertion should pass / fail
...
1. Create a custom `assert_something` function with a call to `assert_create()` or `assert_create_advanced()`
...
Each package has its own features and syntax, and users can choose the one that best fits their needs and preferences. To find the best fit, it can be helpful to try out a few options and consider the speed, default error messages, flexibility, and included assertion capabilities.
